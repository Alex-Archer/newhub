...
Page(filter.loginCheck(true, app, { 
    ...
    new Promise((resolve, reject) => {
      ...
      this.getDataJson("Coach/coachPlanTime", _Config, true).then(val => {
        ...
        _.each(val.list, (x, index) => {
          ...
          {
            //thisTempArr.push(val.list[index]);
            x.use = false;
            x.state = 0;
            thisTempArr.push(x);
          }
  ...
  buttonForm(e) {
    ...
    /*
    if (
      util.isNull(_formStoreID) || _formStoreID == 0 ||
      util.isNull(_formCoachID) || _formCoachID == 0 ||
      util.isNull(_formReserveData) ||
      util.isNull(_formReserveTime) ||
      util.isNull(_formOrderNo)
    ) {
      that.setData({
        PayLoading: false,
        PayLoadingTxt: '',
        ...
        util.toast("数据请选择完整!", null, null, (e) => {
          setTimeout((res) => {
            wx.navigateBack();
          }, 500);
        });
