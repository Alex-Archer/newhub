  ...
  /*
   ...
   getDataJson(_pageIndex = 1) {
    ...
    return new Promise((resolve, reject) => {
        axios.get("Coach/myCertificate", config, {
            headers: {
            ...
            if (res.data.code == 1) {
                ...
                //logs.log("77777777777777777", _data, true)
                resolve(_data);
            } else {
                reject([]);
            }
...
onShow(e){
  ...
  onLoad: function (options) {
    ...
    this.getDataJson(1).then(list => {
        // logs.log("111111111111111",list,true)
        if(list.total>0){
            that.setData({
                storeList:list.data,
                loading:false
  ...
  delController(_id){
    return new Promise((resolve, reject) => {
        let _timestamp = (new Date()).valueOf();
        axios.get("Coach/delCertificate", {
            id: _id,
            userID: wx.getStorageSync('USERID'),
            TIMESTAMP: _timestamp,
        ...
        //if(this.data.refreshParent){//是否需要刷新
            ...
            if (pages.length > 1) //判断页面栈中页面的数量是否有跳转(可以省去判断)
            {
                ...
                prePage.callBackReturn({
                    certificateCount:this.data.storeList?this.data.storeList.length.toString():"0",
                }); //调用上一个页面实例对象的方法
            }   
        //}
    },
