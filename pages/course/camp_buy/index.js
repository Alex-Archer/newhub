...
Page(filter.loginCheck(true, app, {
  ...
  onLoad(options) {
    ...
    this.initLoad(_id).then(val => {
      ...
      that.setData({
        loading: false,
        //subDisabled: false, //可以提交,不行，没有同意协议不给用
      })
      ...
      .then((res) => {
        ...
        if (_data.code === 1 && _data.data.paytime > 0) { //表示支付完成了
          that.setData({
            ...
            util.toast("购买集训营成功！", null, null, (e) => {
              setTimeout((res) => {
                wx.redirectTo({
  ...
  btnPay() {
    ...
    // payAmount: 0, //实付金额
    const _campID = this.data.campID; //团课ID
    ...
    //安检区
    if (util.isNull(_campID) || Number(_campID) <= 0) 
    {
      that.setData({
        PayLoading: false,
        PayLoadingTxt: '',
    ...
    new Promise((resolve, reject) => {
      ...
      let postConfig = {
        ...
        userID: wx.getStorageSync('USERID'),
        TIMESTAMP: timestamp,
        FKEY: md5util.md5(wx.getStorageSync('USERID') + _type + _campID.toString() + _couponSelectID.toString() + timestamp.toString() + app.globalData.APP_INTF_SECRECT)
      }
      ...
      if(_payType===0||_payType===2)
      {
        if (res.code == 1) {
          that.setData({
            ...
            util.toast("集训营兑换成功!", null, null, (e) => {
              setTimeout((res) => {
                wx.redirectTo({
                  url: '/packageA/pages/order/list/index',
                })
          ...
          that.setData({
            ...
            util.toast("集训课兑换失败,请重新操作!", null, null, (e) => {
              setTimeout((res) => {
                wx.navigateBack();
              }, 500);
            });
  ...
  agreeSwitch(e) {
    ...
    that.setData({
        ...
        subDisabled:this.data.agreeChecked
