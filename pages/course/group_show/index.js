...
Page({
  ...
  initLoad(_id = 1) {
    ...
    return new Promise((resolve, reject) => {
      axios.get("Course/detailInfoPub", {
        id: _id,
  ...
  onLoad(options) {
    ...
    this.initLoad(id).then(val => {
      //转换图源给已约用户头像用
      if (val.user && val.user.list) {
        let result = _.map(val.user.list, item => ({
          id: item.id, //编号，非用户真实ID
          src: item.headimgurl
        }));
        that.setData({
          groupInfo: val,
      ...
      that.setData({
        ...
        loading: false,
        subDisabled:(val.price==0||val.order||val.enrollstate==0)?true:false, //可以提交了 ,已购买 和 排除中的除外
  ...
  detailBuy(e){
    ...
    if(util.isNull(this.data.sameTime)){//无时间冲突
        wx.redirectTo({
            url: '/pages/course/groupSubOrder/index?id='+this.data.groupID+'&t='+encodeURIComponent(this.data.groupTitle)
        })
    }else{//有时间冲突
        this.setData({
            modalShow:true
